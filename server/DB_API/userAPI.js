...
async function getUser(data) {
  ...
  // Otherwise, search by the combination of FirstName, LastName, and Email.
    ...
    console.log(
      `Querying user by name and email: ${data.FirstName} ${data.LastName}, ${data.Email}`
    );
    sql =
...
async function updatePassword(data) {
  if (!data.UserID || !data.Password || !data.PasswordSalt) {
    throw new Error(
      "UserID, new Password, and new PasswordSalt are required to update."
    );
  }
  ...
  try {
    ...
    const sql =
      "UPDATE USER SET Password = ?, PasswordSalt = ?, LastLogin = NOW() WHERE UserID = ?";
  ...
  try {
    ...
    if (user.Password === data.Password) {
      ...
      if (user.LastLogin === null) {
        ...
        console.log(
          `LastLogin already set for UserID: ${user.UserID}, not updating`
        );
      }
...
router.post("/addUser", async (req, res, next) => {
  ...
  const UserType =
    source.UserType && !isNaN(Number(source.UserType))
      ? Number(source.UserType)
      : 1;
  ...
  if (
    ...
    return res
      .status(400)
      .json({ message: "Missing required fields.", body: source });
  }
...
router.get("/checkEmail", async (req, res, next) => {
  ...
  if (!email) {
    return res
      .status(400)
      .json({ exists: false, message: "Email parameter is required" });
  }
...
/**
 * Toggles a user's account activity (enable/disable)
