//This file is for admin related database api calls. <-- Orders or Admin calls? -Jason
...
async function AddOrder(data) {
  ...
  try {
    ...
    sql =
      "INSERT INTO ORDERS (ProductID, DriverID, PurchasedAtPrice, OrderStatus, DeliveredOn) VALUES (?, ?, ?, ?, ?)";
    values = [
      data.ProductID,
      data.DriverID,
      data.PurchasedAtPrice,
      "Placed",
      null,
    ];
    console.log("SQL:", sql);
...
router.post("/addOrder", async (req, res, next) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ message: "Order added successfully!", id: result.insertId });
  } catch (error) {
