...
async function createMissingDriverRecords() {
  try {
    console.log(
      "Finding users in SPONSOR_USER who don't have DRIVER records..."
    );
    ...
    console.log(
      `Found ${missingDrivers.length} users missing DRIVER records:`,
      missingDrivers
    );
    ...
    for (const user of missingDrivers) {
      try {
        console.log(
          `Creating DRIVER record for UserID ${user.UserID} (${user.FirstName} ${user.LastName})`
        );

        // Insert directly into DRIVER table instead of using the problematic stored procedure
        const insertDriverQuery =
          "INSERT INTO DRIVER (UserID, SponsorID) VALUES (?, ?)";
        const result = await db.executeQuery(insertDriverQuery, [
          user.UserID,
          user.SponsorID,
        ]);

        console.log(
          `Successfully created DRIVER record with DriverID ${result.insertId} for UserID ${user.UserID}`
        );
        ...
        console.error(
          `Failed to create DRIVER record for UserID ${user.UserID}:`,
          driverCreateError
        );
      }
    }
...
async function debugAllUsers() {
  try {
    ...
    const allUsersQuery =
      "SELECT UserID, FirstName, LastName, Email, UserType, ActiveAccount FROM USER ORDER BY UserID";
    ...
    console.log(
      `Found ${debugData.allSponsorUsers.length} sponsor users in SPONSOR_USER table`
    );

    // Get all sponsors from SPONSOR table
    const allSponsorsQuery =
      "SELECT SponsorID, Name, EnabledSponsor FROM SPONSOR ORDER BY SponsorID";
    debugData.allSponsors = await db.executeQuery(allSponsorsQuery);
    console.log(
      `Found ${debugData.allSponsors.length} sponsors in SPONSOR table`
    );

    // Get the same data that getAllDrivers() returns
    debugData.getAllDriversResult = await getAllDrivers();
    console.log(
      `getAllDrivers() returned ${debugData.getAllDriversResult.length} drivers`
    );

    // Analysis: Find users without driver records
    const userIDsWithDrivers = new Set(
      debugData.allDrivers.map((d) => d.UserID)
    );
    debugData.usersWithoutDriverRecords = debugData.allUsers.filter(
      (user) => user.UserType === 1 && !userIDsWithDrivers.has(user.UserID)
    );

    // Analysis: Find drivers without valid sponsors
    const validSponsorIDs = new Set(
      debugData.allSponsors.map((s) => s.SponsorID)
    );
    debugData.driversWithInvalidSponsors = debugData.allDrivers.filter(
      (driver) => !validSponsorIDs.has(driver.SponsorID)
    );
    ...
    Object.keys(userIDCounts).forEach((userID) => {
      if (userIDCounts[userID] > 1) {
        debugData.duplicateDriverUserIDs.push({
          ...
          drivers: debugData.allDrivers.filter(
    ...
    debugData.summary = {
      ...
      activeUsers: debugData.allUsers.filter((u) => u.ActiveAccount === 1)
        .length,
      inactiveUsers: debugData.allUsers.filter((u) => u.ActiveAccount === 0)
        .length,
      driverTypeUsers: debugData.allUsers.filter((u) => u.UserType === 1)
        .length,
    ...
    console.log(
      "Users with multiple DRIVER records:",
      debugData.duplicateDriverUserIDs
    );
...
router.get("/debugAllUsers", async (req, res, next) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ message: "Error retrieving debug data.", error: error.message });
  }
});
...
async function cleanupDuplicateDriversForUser(userID) {
  try {
    ...
    const getDriversQuery =
      "SELECT DriverID, SponsorID FROM DRIVER WHERE UserID = ? ORDER BY DriverID ASC";
    ...
    console.log(
      `Keeping DriverID ${keepRecord.DriverID}, deleting ${deleteRecords.length} duplicates`
    );
    ...
    if (keepRecord.SponsorID === 0) {
      console.log(
        `Updating SponsorID from 0 to 1 for DriverID ${keepRecord.DriverID}`
      );
      const updateSponsorQuery =
        "UPDATE DRIVER SET SponsorID = 1 WHERE DriverID = ?";
...
async function fixAllDataIntegrityIssues() {
  try {
    ...
    const fixInvalidSponsorsQuery =
      "UPDATE DRIVER SET SponsorID = 1 WHERE SponsorID = 0 OR SponsorID NOT IN (SELECT SponsorID FROM SPONSOR)";
    ...
    return {
      ...
      results: results,
      totalIssuesFixed: results.reduce(
        (sum, r) => sum + (r.deleted ? r.deleted.length : 0),
        0
      ),
    };
  } catch (error) {
...
router.post(
  "/cleanupDuplicateDriversForUser/:userID",
  async (req, res, next) => {
    const userID = req.params.userID;
    console.log(
      "Received request to cleanup duplicate drivers for UserID:",
      userID
    );
    try {
...
async function getDriversForSponsor(sponsorID) {
  try {
    console.log(
      `Reading driver info for SponsorID: ${sponsorID} (including inactive accounts)`
    );
    ...
    console.log(
      `Returning ${sponsorDrivers.length} Drivers for SponsorID ${sponsorID} (including inactive)`
    );
...
async function updateDriverWithSponsor(data) {
  try {
    ...
    console.log(
      `Updating driver UserID ${UserID} with sponsor change capability`
    );
    ...
    if (Password && Password.trim() !== "" && Password !== "auto-generated") {
      const salt =
        PasswordSalt && PasswordSalt !== "auto-generated"
          ? PasswordSalt
          : generateSalt();
    ...
    try {
      const updateSponsorUserSql =
        "UPDATE SPONSOR_USER SET SponsorID = ? WHERE UserID = ?";
      const sponsorUserResult = await db.executeQuery(updateSponsorUserSql, [
        SponsorID,
        UserID,
      ]);
      ...
      if (sponsorUserResult.affectedRows === 0) {
        console.log(
          "No SPONSOR_USER record found to update, creating new one..."
        );
        // If no SPONSOR_USER record exists, create one
        const insertSponsorUserSql =
          "INSERT INTO SPONSOR_USER (UserID, SponsorID) VALUES (?, ?)";
      ...
      console.warn(
        "Failed to update SPONSOR_USER relationship (may not be critical):",
        sponsorUserError.message
      );
    }
