...
/// Gets products for a given category ID
async function getProducts(categoryId, limit = 5) {
...
router.get('/categories', async (req, res) => {
  try {
    ...
    console.log(`[categories] category query="${catName}"`);
...
router.get('/products', async (req, res) => {
  try {
    ...
    console.log(`[products] category query="${catName}", limit=${limit}`);
    ...
    // Take the id from catId and request products
    const products = await getProducts(catId, limit);

    // Mapping a stable img url
    const normalized = products.map(p => ({
      // Normalize product data and select the best available image URL
      sku: p.sku,
      name: p.name,
      salePrice: p.salePrice,
      // Go through the different img options prioritizing the largeImage
    ...
    // Response with the normalized products
    res.json({ 
      categoryId: catId, 
      category: catName,
      products: normalized,
...
router.get('/health', (req, res) => {
  res.json({ 
    status: 'ok', 
    service: 'Best Buy API Router',
    apiKeyConfigured: !!API_KEY
  });
});

module.exports = { router, findCatId, getProducts };