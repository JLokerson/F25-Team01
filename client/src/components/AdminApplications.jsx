...
export default function AdminApplications() {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        console.log(
          "Using shared fallback application data - API endpoints not available"
        );
  ...
  const handleConfirmAction = async () => {
    ...
    if (actionType === "approve") {
      console.log(
        `Admin approving application for ${selectedApplication.firstName} ${selectedApplication.lastName}`
      );
      ...
      console.log(
        "Creating driver with USER and DRIVER table records:",
        driverData
      );
      console.log(
        "Expected DRIVER table entry: { DriverID: auto-generated, SponsorID:",
        selectedApplication.sponsorId,
        ", UserID: auto-generated, Points: 0 }"
      );
      ...
      try {
        ...
        const response = await fetch(
          `http://localhost:4000/driverAPI/addDriver?${queryString}`,
          {
            method: "POST",
            headers: {
      ...
      console.log(
        `Admin denying application for ${selectedApplication.firstName} ${selectedApplication.lastName}`
      );
  ...
  const filteredApplications = applications.filter((app) => {
    ...
    const orgMatch =
      ...
      app.requestedOrganization === filterOrganization;
    return statusMatch && orgMatch;
  });

  // Get unique organizations for filter dropdown
  const uniqueOrganizations = [
    ...new Set(applications.map((app) => app.requestedOrganization)),
  ];
            ...
            {filteredApplications.length === 0 ? (
              <div className="alert alert-info">
                ...
                {filterStatus === "pending" && (
                  <div className="mt-2">
                    <small>
                      To view processed applications, change the status filter
                      above.
                    </small>
                  </div>
                )}
            ...
            <div className="modal-content">
              ...
              <div className="modal-body">
                <div className="row">
                  ...
                  <div className="col-md-6">
                    <h6>
                      <i className="fas fa-id-card me-2"></i>Driver &
                    ...
                    <p>
                      <strong>Requested Organization:</strong>
                      <span className="badge bg-info ms-2">
                        {selectedApplication.requestedOrganization}
                      </span>
                    </p>
                    <p>
                    ...
                    {selectedApplication.status === "denied" && (
                      <div className="mt-2 p-2 bg-danger bg-opacity-10 rounded">
                        <small>
                          ...
                          {selectedApplication.deniedBy}
                          <br />
                          <strong>Denied on:</strong>{" "}
                          {new Date(
                            selectedApplication.deniedDate
                          ).toLocaleDateString()}
                          <br />
                ...
                {actionType === "approve" && (
                  <div className="mt-3">
                    <div className="alert alert-info">
                      <h6>
                        <i className="fas fa-info-circle me-2"></i>Approval
                        Information
                      </h6>
                      <p className="mb-1">
                        A new user account will be created with the following
                        details:
                      </p>
                      <ul className="mb-0">
                        <li>
                          <strong>User Type:</strong> Driver (Type 1)
                        </li>
                        <li>
                        ...
                        <li>
                          <strong>Action By:</strong> Admin (System-wide
                          approval)
                        </li>
                      </ul>
                      <small className="text-muted">
                ...
                {actionType === "deny" && (
                  <div className="mt-3">
                    <label htmlFor="denialReason" className="form-label">
                      <strong>
                        <i className="fas fa-comment-slash me-1"></i>Reason for
                        Denial:
                      </strong>
                    </label>
                    <textarea
                ...
                {!actionType ? (
                    ...
                    {selectedApplication.status === "pending" && (
                      <>
                        <button
                          ...
                          onClick={handleDeny}
                        >
                          <i className="fas fa-times-circle me-1"></i>Deny
                          Application
                        </button>
                        <button
                          type="button"
                  ...
                  <>
                    ...
                    >
                      <i
                        className={`fas ${
                          actionType === "approve" ? "fa-check" : "fa-times"
                        } me-1`}
                      ></i>
