      ...
      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 12 }}>
        <label style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
          ...
          <select value={mode} onChange={(e) => setMode(e.target.value)} disabled={disabled}
                  style={{ padding: '6px 8px', borderRadius: 8, background: '#111', color: '#fff' }}>
          ...
          <>
            <label style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
              ...
              <input type="number" min={1} value={page}
                     ...
                     disabled={disabled}
                     style={{ width: 80, padding: '6px 8px', borderRadius: 8, background: '#111', color: '#fff' }} />
            </label>
          </>
        )}
        {mode !== 'all' && (
          <label style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            ...
            <input type="number" min={1} max={100} value={pageSize}
                   onChange={(e) => setPageSize(Math.max(1, Math.min(100, Number(e.target.value) || 1)))}
                   disabled={disabled}
                   style={{ width: 100, padding: '6px 8px', borderRadius: 8, background: '#111', color: '#fff' }} />
          </label>
        )}
        {mode === 'cursor' && (
          <>
            <label style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
              Cursor:
              <input value={cursor} onChange={(e) => setCursor(e.target.value)} disabled={disabled}
                     style={{ width: 360, padding: '6px 8px', borderRadius: 8, background: '#111', color: '#fff' }} />
            </label>
            <button onClick={onResetCursor} disabled={disabled}
                    style={{ padding: '6px 10px', borderRadius: 8, background: '#374151', color: '#fff', border: '1px solid #4b5563' }}>
              Reset to *
            </button>
            <button onClick={onNextCursor} disabled={disabled || !nextCursor}
                    title={nextCursor || ''}
                    style={{ padding: '6px 10px', borderRadius: 8, background: '#1d4ed8', color: '#fff', border: '1px solid #3b82f6' }}>
              Use nextCursor
            </button>
          </>
        )}
        <button onClick={fetchCategories} disabled={disabled}
                style={{ padding: '6px 10px', borderRadius: 8, background: '#16a34a', color: '#fff', border: '1px solid #22c55e' }}>
        ...
        <button onClick={downloadAllNames} disabled={loading || downloading}
                title="Download JSON array of category names only"
                style={{ padding: '6px 10px', borderRadius: 8, background: '#0ea5e9', color: '#fff', border: '1px solid #38bdf8' }}>
        ...
        {items.map((c) => (
          <React.Fragment key={c.id}>
            ...
            <div>
              ...
              {c.url && (
                <div>
                  <a href={c.url} target="_blank" rel="noopener noreferrer" style={{ color: '#93c5fd' }}>
                    {c.url}
                  </a>
                </div>
              )}
